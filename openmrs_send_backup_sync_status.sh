...
# cd  backup dir
cd /data_ssd_1/openmrs_backups

# Clear text 
echo "+-----------------+-------------+---------------------------------------------------+"  >  /data_ssd_1/scripts/openmrs_sync.txt
echo "| U. Sanitaria    |  Ult Backup | OpenMRS Link                                      |"  >> /data_ssd_1/scripts/openmrs_sync.txt
echo "+-----------------+-------------+---------------------------------------------------+"  >> /data_ssd_1/scripts/openmrs_sync.txt
...
# print to file
  if [[ " ${group5[*]} " =~ " ${us} " ]]; then
    ...
    echo "| "$us"           |  "$var1"            |"  >> /data_ssd_1/scripts/openmrs_sync.txt
  elif [[ " ${group6[*]} " =~ " ${us} " ]]; then
    # whatever you want to do when array contains value
    echo "| "$us"          |  "$var1"           |"   >> /data_ssd_1/scripts/openmrs_sync.txt
  elif [[ " ${group7[*]} " =~ " ${us} " ]]; then
...
#if your want to see the output just uncomment this line
